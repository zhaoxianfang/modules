...
仅缓存模块元数据还不够：默认情况下，每个请求/命令都会对**每个模块**的目录执行
`File::files()` 扫描、`class_exists()` 自动加载探测与 `ReflectionClass` 反射，
用于自动发现服务提供者、命令、观察者、策略、仓库、事件等组件。模块数量较多时，
这部分 IO 与 CPU 开销会成为性能瓶颈。

开启 `MODULES_CACHE_ENABLED=true` 后，上述「发现结果（清单）」会在首次发现后
持久化到 `storage/framework/cache/modules/discovery.php`，后续请求直接读取清单并注册，
**完全跳过目录扫描与自动加载探测**。清单与模块元数据缓存同生命周期：

- `php artisan module:cache`：重建元数据缓存，并**预热**所有已启用模块的发现清单；
- `php artisan module:clear`：同时清除元数据与发现清单缓存；
- 清单读取/解析失败时自动回退到实时扫描，绝不阻断启动；
- 部署新增组件后请重新执行 `module:cache`，否则新组件需等待清单重建才会被发现。

> 提示：路由文件与配置文件仍需在每个请求合并（Laravel 运行时状态不持久化），
> 但其扫描开销已通过清单缓存消除；视图/迁移/翻译命名空间注册也改为基于缓存标记，
> 不再每次请求重复 `is_dir` 探测。

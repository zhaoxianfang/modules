...
**重要说明**：
- ⭐ **v5.0 起**：不再需要每个模块维护 `composer.json`，所有元数据均在此文件中定义
- `enabled` 键控制模块的启用/禁用，设为 `false` 则完全跳过加载
- `priority` 控制模块加载顺序，数字越小越优先
- `providers` 和 `laravel_aliases` 会自动注册到 Laravel 容器
- 自定义配置通过 `options` 键管理
...
$value = module_config('options.key', 'default');

// 方式 2：使用智能配置读取
$value = module_config('common.name', 'hello');

// 方式 3：使用 config 函数
$value = config('blog.options.key', 'default');

// 方式 4：使用模块实例
$module = module('Blog');
$value = $module->config('options.key', 'default');
```
...
通过修改 `Config/{lower_name}.php` 中的 `enabled` 键：
...
> **注意**：v5.0 将旧版 `enable` 键更名为 `enabled`，旧键不再支持。
...
- 使用模块配置文件集中管理元数据（替代 composer.json）
- 提供合理的默认值
- 文档化所有配置选项
